...
<html>
	...
	<body bgcolor="#E0FFFF">
    	<center>
			<table border="0" width="1003" height="299" cellpadding="0" cellspacing="0"><tr><td background=""></td></tr></table>
	   		<c:set var="artReview" value="${requestScope.artrlist}"/>
			
			<c:if test="${empty artReview}">
			<table border="10" width="1003" height="167" background="i" bordercolor="#4E6900" style="margin-top:1;margin-bottom:1"><tr><td align="center">该文章目前没有任何评论哦！</td></tr></table>
			</c:if>
			
       		<c:if test="${!empty artReview}"> 
       		<c:set var="cpage" value="${requestScope.createPage}"/>      	
		       	<table border="1" width="1003" rules="none" cellpadding="0" cellspacing="0" bordercolor="#4E6900" bordercolordark="white" bordercolorlight="#4E6900" background="i" style="word-break:break-all">
		       		<tr height="112">
		       			<td width="55%" background="" style="padding-left:5">文章所有评论    <font color="grey">共${cpage.allR}条</font></td>
		       			<td align="right" background=""><c:out value="${requestScope.message}" escapeXml="false"/>
		       		</tr>
	       			<c:forEach varStatus="st" var="single" items="${artReview}">
	       			<tr>
	       				<td style="padding-left:8"><font color="blue">${single.artRAuthor}</font></td>
